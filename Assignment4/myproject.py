    ...
    if username in [my_slack_username, 'zac.wentzell']:
        ...
        if text == '&lt;BOTS_RESPOND&gt;':
            response = {
                ...
                'icon_emoji': ':pill:',
                'text': 'Hello, my name is zlin_bot. I belong to zlin15. I live at 52.43.135.7.(Waited for {}s before responding)'.format(delay)}
       ...
       # In task3, the question may have to end with '?' and no space between tags.
        ...
        if "&lt;WHAT'S_THE_WEATHER_LIKE_AT&gt;:" in text:
            ...
            if var1.isdigit() != True:
                ...
                if len(var1)==5:
                    var1 = var1+',us'
                    get = requests.get(url='http://api.openweathermap.org/data/2.5/weather?',
                                       params={'q': var1, 'appid': 'b41bd0a566eba94f06bbf9505f58e24d'})
                else:
                    get = requests.get(url='http://api.openweathermap.org/data/2.5/weather?',
                                       params={'q': var1, 'appid': 'b41bd0a566eba94f06bbf9505f58e24d'})
            ...
            main_temp= result['main']['temp']   # Get temperature(Kelvin type)
            temp = (main_temp*1.8)-459.69    #  Transform temperature from Kelvin to Fahrenheit
            wind_speed = result['wind']['speed']  # Unit: meter/second
            humidity = result['main']['humidity']  # Unit : %
            current_time = int(round(time.time()))   # Get the current time
            response={
                "username": my_bot_name,
                "icon_emoji": ":taxi:",
                "attachments": [
